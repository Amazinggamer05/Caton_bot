...
@client.command(name = "spek", definition = "spits actual fire.", cooldown = 3)
async def spek(message):
    if VaRi == 0:
        await message.channel.send(simpDiss[rand(0, len(simpDiss))])
    if VaRi == 1:
        await message.channel.send(imBest[rand(0,len(imBest))])
